...
void customPrint({String? name, required dynamic data}) {
  _printTop();
  _printName(name);
...
void _printName(String? name) {
  if (name != null) {
    final List<String> nameList =
        wrapText(name, shouldWrap: true, columnWidth: 76).split('\n');
    _printLine(nameList);
    debugPrint(
      '├──────────────────────────────────────────────────────────────────────────────┤',
    );
...
void _printData(String data) {
  final List<String> dataList =
      wrapText(data, shouldWrap: true, columnWidth: 76).split('\n');
  _printLine(dataList);
}
  ...
  for (final String line in splitText) {
    ...
    result.addAll(notIndented.map<String>(
      (String line) {
        ...
        if (truncatedIndent.length > columnWidth - 10) {
          truncatedIndent =
              truncatedIndent.substring(0, math.max(columnWidth - 10, 0));
        }
...
List<String> _wrapTextAsLines(
  ...
  final List<String> result = <String>[];
  final int effectiveLength = math.max(columnWidth - start, 10);
